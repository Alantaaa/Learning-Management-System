...
export default function SignInPage() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!userData?.role) {
        alert("Error: Role tidak ditemukan. Hubungi admin.");
        return;
      }

      secureLocalStorage.setItem(STORAGE_KEY, JSON.stringify(userData));
      ...
      if (userData.role === "manager") {
        ...
        alert("Error: Role tidak dikenali");
      }
    } catch (error) {
      alert("Sign-in gagal: " + (error.message || "Coba lagi"));
    }
  };
  ...
  return (
    <div className="relative flex flex-col flex-1 p-[10px]">
      ...
      <nav className="flex items-center justify-between p-[30px]">
        <div className="text-white font-bold text-xl">ShaynaLMS</div>
      ...
      >
        ...
        <button
          disabled={isLoading}
          type="submit"
          className="w-full rounded-full border p-[14px_20px] text-center font-semibold text-white bg-[#662FFF] border-[#8661EE] shadow-[-10px_-6px_10px_0_#7F33FF_inset] hover:opacity-90 disabled:opacity-50 transition-opacity"
        >
          {isLoading ? "Loading..." : "Sign In to Manage"}
        </button>
      </form>
    </div>
  );
